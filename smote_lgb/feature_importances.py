...
#     plt.title(f"{db_name} ",  fontsize=24)
    fig_fi.savefig(f"./result_cutdata_{file_name}/smote_trainvaldata/opt_auc/data_{min_num}_{max_num}/cv_{fold_strategy}_valsplit_{val_fold_strategy}/es_{early_stopping_num}_val_fold_{val_fold}/feature_importances_{file_name}_mor{len(mor_class)}_{fold_strategy}.png")
    return 
...
df_fi.to_csv(f"./result_cutdata_{file_name}/smote_trainvaldata/opt_auc/data_{min_num}_{max_num}/cv_{fold_strategy}_valsplit_{val_fold_strategy}/es_{early_stopping_num}_val_fold_{val_fold}/feature_importances_{file_name}_mor{len(mor_class)}_{fold_strategy}.csv", index = False)
