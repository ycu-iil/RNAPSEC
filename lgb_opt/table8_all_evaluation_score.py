...
for file_name in file_name_list:
    ...
    #             print(max_num, fold_strategy, val_fold_strategy)
                path = glob.glob(f"./result_cutdata_{file_name}/data_{min_num}_{max_num}/opt/cv_{fold_strategy}_valsplit_{val_fold_strategy}/es_{early_stopping_num}_val_fold_{val_fold}/opt_auc/score.csv")
                if len(path) > 0:
...
df_total_score.to_csv("./honbun_result/all_results_dir.csv")