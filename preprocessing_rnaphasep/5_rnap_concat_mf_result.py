...
df_kmer = df_kmer[pd.to_numeric(df_kmer.A, errors="coerce").notna()]
#df_filtered_kmer = df_kmer.loc[:, ['A', 'C', 'U', "G", 'ACG', 'AGA', 'AGU', 'AUC', 'CCG', 'CGC', 'GAC', 'GCA', 'GCC', 'GGG', 'GUC', 'UAC', 'UGU']]
...
#filterなし、RNAのkmer全部
unfil_kmer = df_kmer.copy()
for i in unfil_kmer.columns:
    unfil_kmer = unfil_kmer.rename(columns = { f"{i}" : f"rna_{i}"})
unfil_kmer.to_csv("./prepro_results/rna_total_kmer.csv", index = False)

#concat, rename
#df_rna_features=pd.concat([df_filtered_kmer, df_fickett2, df_tsallis3, df_fourier, df_shanon, df_orf_used, ], axis="columns")
df_rna_features=pd.concat([unfil_kmer, df_fickett2, df_tsallis3, df_fourier, df_shanon, df_orf_used, ], axis="columns")
...
df_rna_features_2.to_csv("./prepro_results/rna_mf_results.csv", index=False)
