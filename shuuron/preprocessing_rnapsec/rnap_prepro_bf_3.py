...
def seq2features(seq_dict):
    ...
    pd_feature_aa=pd.DataFrame([[np.nan]* len(df_aa_analysis1.columns)]*df.shape[0],columns=df_aa_analysis1.columns)
    pd_feature_aa=pd_feature_aa.fillna(df_aa_analysis1)
    return pd_feature_aa

df = pd.read_csv("./preprocessing_results/rnap_preprocessing_1.csv")
seq_dict = dict(df.aa)
df_bf = seq2features(seq_dict)
df_protein_bf = df_bf.copy()
for i in df_bf.columns:
    df_protein_bf = df_protein_bf.rename(columns = { f"{i}" : f"protein_{i}"})
df_protein_bf.to_csv("./preprocessing_results/rnap_preprocessing_bf.csv", index = False)
