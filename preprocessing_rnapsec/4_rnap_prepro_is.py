...
def calc_salt2(df, df_salt, salt_conc2, salt_unit2):
    ...
    df_salt2_is=df_salt2_is.rename(columns={df_salt2_is.columns[0]:0})
    
    return df_salt1_is, df_salt2_is

def calc_salt3(df, salt_conc, salt_unit):
    "3種塩"
    ...
    for i in salt_unit3.index:
        ...
        #h: 3つ目の塩が2価の塩
        assert name3_0[(name3_0.str.contains("hospha"))| (name3_0.str.contains("NaCl"))].shape[0]== name3_0.shape[0], f"other salt included in name3_0: {name3_0.unique()}" # AttributeError: 'str' object has no attribute 'contains'
        salts = []
        if name3_0.loc[i]=="NaCl" :
            ...
            if (name3_1.loc[i] == "KCl") & (name3_2.loc[i] == "MgCl2"):
                ...
                print("other salt?")
                print(0, name3_0.loc[i])
                print(1, name3_1.loc[i])
                print(2, name3_2.loc[i])
        #塩が計算できないやつ（尿素？とか）
        else:
            salt3_is.append(salt_a.get_ionic_strength())
            salt3_index.append(i)
...
def main():
    ...
    df = df.rename(columns = {"salt_unit/salt_name": "salt_unit_name"})
    df.pH[df.pH =="-"] = 7.4
    df.pH = df.pH.astype("float")
    df.salt_conc = df.salt_conc.replace({"-":np.nan}, )
    df.salt_conc = df.salt_conc.replace({None:np.nan})
    df.salt_unit_name = df.salt_unit_name.str.replace("ﾎｼ|μ", "u")
    df.salt_unit_name[df.salt_unit_name.str.contains("http", na = False)] = "mM/KCl, mM/MgCl2"
    df.salt_unit_name = df.salt_unit_name.replace({None:np.nan})
    df_salt = pd.DataFrame(df.salt_conc)
    df_salt["salt_unit"] = df.salt_unit_name
    df_salt["pH"] = df.pH.astype("float")
    ...
    if df_salt.salt_unit.str.count(",").unique().shape[0] == 5:
        df_salt1_is, df_salt2_is = calc_salt2(df, df_salt, salt_conc2, salt_unit2)
        df_salt3_is = calc_salt3(df, salt_conc, salt_unit)
        is_list = [df_salt1_is, df_salt2_is, df_salt3_is]
    elif df_salt.salt_unit.str.count(",").unique().shape[0] == 4:
        df_salt1_is, df_salt2_is = calc_salt2(df, df_salt, salt_conc2, salt_unit2)
        is_list = [df_salt1_is, df_salt2_is,]
    "計算したイオン強度を大元のデータフレームにカラム追加する"

    df_salt_is = pd.concat(is_list, axis = "index")
    df["ionic_strength"] = df_salt_is

    df.ionic_strength = df.ionic_strength.fillna(0)
    df.ionic_strength[df.salt_unit_name.str.contains("phosphate", na = False)] = np.nan
    df.to_csv("./preprocessing_results/rnap_preprocessing_is.csv", index = False)
