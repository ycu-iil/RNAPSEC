...
def x_use_col(df):
    X = df.loc[:, ['protein_weight', 'protein_A', 'protein_C', 'protein_D', 'protein_E',
       ...
       'rna_cv_ORF_length',"rna_conc_log", "protein_conc_log", "pH", "temp", "ionic_strength","aa_rna_label", "aa_label"]]
    return X
def read_file():
...
df_ex = pd.read_csv("./prepro_results/preprocessing_result_ex.csv").reset_index(drop = True)
df_label = pd.read_csv("./prepro_results/preprocessing_result_ex_2.csv").reset_index(drop = True)
assert df_rna.shape[0] == df_b.shape[0] == df_ex.shape[0]== df_input.shape[0], "shape of input files did not matched"
df = pd.concat([ df_ex, df_b, df_rna, df_label.loc[:, ["aa_label", "aa_rna_label"]]], axis = 1)
